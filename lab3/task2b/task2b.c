...
void kill_virus(char *fileName, int signitureOffset, int signitureSize){
    ...
    unsigned char value[signitureSize];
    ...
    if (fseek(f, signitureOffset, SEEK_SET) == 0)
    {
        printf("%d\n",fwrite(value,sizeof(value),1,f));
        printf("File fixed!");
    }
...
void fix(char *fileName){
    ...
    int startingByte;
    int sigSize;
    printf("Starting byte location in file:");
    fgets(option,5,stdin);
    sscanf(option,"%d", &startingByte);
    printf("\nVirus signature size in bytes:");
    fgets(option,5,stdin);
    sscanf(option,"%d", &sigSize);
    printf("\n");

    kill_virus(fileName,startingByte,sigSize);
}
